module.exports = class GrammarListView {
  constructor() {
    ...
    this.selectListView = lumine.workspace.buildSelectList({
      ...
      // The grammar itself, so the rule under the active row survives two
      // grammars sharing a scope name — which is exactly what the list shows
      // when duplicates are not hidden. Null under a query: the rows are
      // ranked by score there, and a line drawn in that would mean nothing.
   ...
   * happens when hiding duplicates dropped the exact grammar in favour of its
   * twin.
  ...
  async toggle(event) {
    ...
    const editor = element?.getModel?.() ?? lumine.workspace.getActiveEmbeddedTextEditor();
    if (editor) {
      this.editor = editor;
...
function getLanguageModeConfig() {
  let isTreeSitterMode = lumine.config.get("editor.useTreeSitterParsers");
...
function getParserPreferenceForScopeName(scopeName) {
  let useTreeSitterParsers = lumine.config.get("editor.useTreeSitterParsers", {
    scope: [scopeName],
  });
